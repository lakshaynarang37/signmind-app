...
const ResearchHub = () => {
  ...
  const load = useCallback(async (variant = refreshRef.current) => {
    setLoading(true);
    try {
      const d = await getResearchArticles(variant);
      setArticles(d.items || []);
      setSource(d.source || "");
      setUpdatedAt(new Date());
    } catch {
      // Keep the previous set of articles if the network blips.
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const refresh = useCallback(() => {
    const next = refreshRef.current + 1;
    ...
    <div className="space-y-5">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Peer-reviewed research on Deaf mental health, psychotherapy, and
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs text-muted-foreground">
          Auto-refreshes every 5 minutes and rotates the search focus.
        </span>
      </div>

      <div className="text-[11px] text-muted-foreground">
        Feed rotation {refreshIndex}{" "}
        {updatedAt
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    {article.url && (
                      <a
                        ...
                        className="shrink-0 p-1.5 rounded-lg hover:bg-accent text-muted-foreground hover:text-violet-400 transition-colors"
                      >
      ...
      <div className="text-center text-xs text-muted-foreground/50 pt-2">
        Articles are sourced from Europe PubMed Central and refreshed on a timer
      </div>
    </div>
  );
};
