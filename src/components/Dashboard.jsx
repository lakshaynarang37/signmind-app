  ...
  >
    <Card className="border-border/50 relative overflow-hidden hover:border-border transition-colors duration-200">
      <CardContent className="p-5">
        ...
        <p
          className={`text-2xl font-display font-bold ${color.text} tracking-tight`}
        >
          {value}
        </p>
        <p className="text-xs text-muted-foreground mt-1">{label}</p>
        <div
          className={`absolute bottom-0 right-0 w-16 h-16 rounded-full ${color.glow} blur-2xl opacity-25`}
        />
      </CardContent>
    </Card>
  </motion.div>
...
const QuickAction = ({ icon: Icon, label, desc, color, onClick, delay }) => (
  ...
  >
    ...
    >
      <CardContent className="p-5">
        <div
          className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 ${color.bg} group-hover:scale-105 transition-transform duration-200`}
        >
...
const Dashboard = ({ currentUser, setActiveTab, token }) => {
  const [wellness, setWellness] = useState({
    insight: "You have been showing up for yourself consistently.",
    affirmation: "Small steps still matter.",
    weather: {},
  });
  ...
  const dayMoods = DAYS.map((day, i) => {
    ...
    const dayLogs = moodData.filter(
      (m) => new Date(m.loggedAt).toDateString() === dateStr,
    );
    const avg = dayLogs.length
      ? dayLogs.reduce((s, m) => s + m.score, 0) / dayLogs.length
      : null;
    const colors = [
      "hsl(252 85% 68%)",
      "hsl(174 72% 47%)",
  ...
  return (
    <div className="space-y-5">
            ...
            <div className="flex-1">
              ...
              <p className="text-muted-foreground text-sm leading-relaxed mb-5 max-w-lg">
                ...
                {wellness.weather?.temperatureC != null
                  ? ` It's ${wellness.weather.temperatureC}°C outside.`
                  : ""}
              </p>
              <div className="flex flex-wrap gap-2">
                ...
                >
                  Open Support Chat <ArrowUpRight size={14} />
                </Button>
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <StatCard
          delay={3}
          value={Math.max(moodData.length, 0)}
          label="Mood Logs"
          icon={Activity}
          color={{
            bg: "bg-violet-500/10",
          ...
          <Card className="border-border/50">
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground/60 mt-3 text-center">
                Log your mood daily to see real data here
              </p>
            </CardContent>
          </Card>
        </motion.div>
            ...
            <CardContent>
              {[
                ...
                >
                  ...
                  <span
                    className={`text-xs flex-1 ${task.done ? "line-through text-muted-foreground" : "text-foreground"}`}
                  >
                    {task.label}
                  </span>
                </div>
