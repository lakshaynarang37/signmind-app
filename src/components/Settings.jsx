...
import { 
  User, Moon, Sun, Bell, Shield, LogOut, ChevronRight, 
  Palette, Globe, Lock, CreditCard, HelpCircle, Mail, Save, X, Loader2
} from "lucide-react";
import { cn } from "../lib/utils";
import { updateProfile, updatePassword } from "../services/backendApi";

const THEMES = [
  { id: "midnight", name: "Midnight Obsidian", color: "#94d8c0", desc: "Sage & Deep Charcoal" },
  { id: "emerald", name: "Emerald Glade", color: "#5eead4", desc: "Teal & Dark Forest" },
  { id: "amber", name: "Amber Harvest", color: "#fcd34d", desc: "Ochre & Warm Black" },
  { id: "sunset", name: "Dusty Sunset", color: "#fda4af", desc: "Rose & Earthy Dark" }
];

export default function Settings({ token, currentUser, onLogout, theme, setTheme }) {
  ...
  return (
    <div className="space-y-8 pb-20 lg:pb-0 relative">
      ...
      <div className="space-y-4">
        <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground px-2">Account Security</h3>
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-left">
                    <p className="text-sm font-semibold text-foreground">{currentUser?.name || "Global User"}</p>
                    <p className="text-xs text-muted-foreground">{currentUser?.email || "user@signmind.com"}</p>
                  </div>
                </div>
                <div className="text-xs font-bold text-primary opacity-0 group-hover:opacity-100 transition-opacity">Edit Profile</div>
              </button>
            )}
          </div>
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-xs text-muted-foreground">Change your password and manage sessions</p>
                  </div>
                </div>
                <div className="text-xs font-bold text-primary opacity-0 group-hover:opacity-100 transition-opacity">Change</div>
              </button>
            )}
          </div>
        </div>
      ...
      <div className="pt-4">
        ...
        <p className="text-center text-[10px] text-muted-foreground mt-4">
          SignMind Wellness Platform v2.0 • Secured with CodeRabbit AI
        </p>
      </div>
    </div>
  );
